...
public class PlateformMerchanApplyComponent extends BaseComponent {
     ...
     */
    public PageData<PlateformMerchanApply> queryApplysForApplyCheck(Integer applyState, OperatorInfo oi,
                                                                    Map<String, Object> appendParam,
                                                                    Integer pageNo, Integer pageSize) throws Exception {
        if (applyState.equals(apply_state_waitcheck) || applyState.equals(apply_state_suc)
                || applyState.equals(apply_state_failed)) {
            if (!oi.isSupperOpe()) {
                appendParam.put("operatorCode", oi.getOperatorCode());
            }
        }
        return applyList(appendParam, pageNo, pageSize);
    }
    ...
    public Map applyDistribute(Integer applyId, String opeCode, OperatorInfo oi) throws BusiException {
        ...
        if (oi == null || oi.getOperatorCode() == null || oi.getOperatorCode().length() <= 0)
            throw new BusiException("未找到指定的操作员");
        PlateformMerchanApply updateDto = new PlateformMerchanApply();
        updateDto.setId(apply.getId());
        updateDto.setOpeCode(oi.getOperatorCode());
        updateDto.setOpeName(oi.getOperatorName());
